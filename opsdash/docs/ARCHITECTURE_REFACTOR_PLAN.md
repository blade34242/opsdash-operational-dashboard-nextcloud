...
1. **Introduce Composables**
   ...
   - Status (2025-10): `useDashboard`, `useCategories`, `useCharts`, `useSummaries`, and `useBalance` now concentrate the dashboard data flow and derived metrics. `App.vue` primarily wires composable outputs and pane props; persistence queuing still sits in `useDashboard`.
   - Next slice: migrate `queueSave`/persistence into the composable once additional tests cover selection flows.
