...
function normalizeBalanceConfig(input: any, categories: TargetCategoryConfig[], base: BalanceConfig): BalanceConfig {
  ...
  const baseIndex = base.index ?? { method: 'simple_range', basis: 'category' }
  let basis: BalanceConfig['index']['basis'] = 'category'
  if (typeof indexInput?.basis === 'string') {
    const raw = indexInput.basis.toLowerCase()
    if (raw === 'calendar' || raw === 'both' || raw === 'off') {
      basis = raw
    } else if (raw === 'category') {
      basis = 'category'
    }
  } else if (baseIndex?.basis === 'calendar' || baseIndex?.basis === 'both' || baseIndex?.basis === 'off') {
  ...
  return {
    ...
    index: {
      method: indexInput?.method === 'shannon_evenness' ? 'shannon_evenness' : baseIndex.method,
      basis,
    },
    thresholds: {
