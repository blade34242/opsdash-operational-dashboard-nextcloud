...
- Admins can enable aggregate usage metrics (open/load/save) in Settings → Administration → Calendar Dashboard.
- Counters are aggregate only (no user data). You can reset them anytime.
- Diagnostics for calendar query shape (structured vs ICS) are logged only when server log level is set to Debug.
