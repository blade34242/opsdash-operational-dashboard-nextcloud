...
final class ConfigDashboardController extends Controller {
    ...
    public function index(): TemplateResponse {
        ...
        if ($this->metrics->enabled()) {
            $this->metrics->increment('open');
            $uid = (string)($this->userSession->getUser()?->getUID() ?? '');
            if ($uid !== '') $this->metrics->incrementUser($uid, 'open');
        }
    ...
    public function load(): DataResponse {
        ...
        if ($this->metrics->enabled()) {
            $this->metrics->increment('load');
            if ($uid !== '') $this->metrics->incrementUser($uid, 'load');
        }
        ...
        if ($save) {
            try { $this->config->setUserValue($uid, $this->appName, 'selected_cals', implode(',', $selectedIds)); }
            catch (\Throwable $e) { $this->logger->error('Config save failed: '.$e->getMessage(), ['app'=>$this->appName]); }
        }
        ...
        return new DataResponse([
            'ok'   => true,
            'meta' => ['uid'=>$uid,'range'=>$range,'offset'=>$offset,'from'=>$from->format('Y-m-d'),'to'=>$to->format('Y-m-d')],
            'calendars' => $sidebar,
    ...
    public function save(): DataResponse {
        ...
        if ($this->metrics->enabled()) {
            $this->metrics->increment('save');
            if ($uid !== '') $this->metrics->incrementUser($uid, 'save');
        }
        $method = strtoupper((string)($_SERVER['REQUEST_METHOD'] ?? 'GET'));
        $data = null;
        if ($method === 'POST') {
            ...
            $data = ['cals' => $this->request->getParam('cals', [])];
        }
